...
\begin{ejercicio}
    ...
    \end{equation*}
    Que es un camino que une $x$ con $y$. Como $x$ e $y$ eran arbitrarios, podemos unir cualesquiera dos puntos de $\bigcup\limits_{i \in I}A_i$, por lo que dicho conjunto es arcoconexo. El proceso está representado en la Figura~\ref{fig:union_arcoconexos}.
...
\begin{ejercicio}
    ...
    \begin{itemize}
        ...
        Notemos que es importante que $\alpha(\nicefrac{1}{2}) = x$. Si hubiéramos definido $\alpha(\nicefrac{1}{2}) = x_0$, entonces $\alpha^{-1}(U) = \left[\nicefrac{1}{2},1\right]$, que no es abierto en $[0,1]$, por lo que $\alpha$ no sería continua.
...
\begin{ejercicio}
   ...
   \noindent
   Sea $U$ un abierto conexo de $(\mathbb{R}^n, \cc{T}_u)$. Gracias al Teorema~\ref{teo:arcoconexo_entorno}, falta ver que todo punto suyo admite un entorno arcoconexo en la topología inducida en $U$ para ver que $U$ es arcoconexo. Para ello, sea $x\in U$, como $U$ es abierto existe $r\in \mathbb{R}^+$ de forma que $B(x,r)\subset U$. $B(x,r)$ es un conjunto arcoconexo por ser convexo, luego es un entorno arcoconexo de $x$ en $U$. Como $x$ era un punto arbitrario de $U$, todo punto suyo admite un entorno arcoconexo, y como $U$ era conexo, tenemos que $U$ es arcoconexo.\\

   \noindent
   Ahora, veamos que no es cierto que todo cerrado conexo de $\mathbb{R}^n$ es arcoconexo. Consideramos $f:\mathbb{R}^+\to \mathbb{R}$ dada por:
   \begin{equation*}
       f(x) = \sen\left(\dfrac{1}{x}\right) \qquad \forall x\in \mathbb{R}^+
   \end{equation*}
   Consideramos el conjunto $C=\ol{Gr(f)}$, mostrado en la Figura~\ref{fig:adherencia_sin1x}, y veamos que es cerrado y conexo pero no arcoconexo.
   \begin{equation*}
       ...
       \centering
        \begin{tikzpicture}
          \begin{axis}[
            ...
            xmin=0, xmax=1,
            ymin=-1.2, ymax=1.2,
            samples=400 % menos puntos = compila más rápido
          ]
        ...
        \label{fig:adherencia_sin1x}
   \end{figure}
   En primer lugar, por definición $C$ es cerrado. Además, en Topología I vimos que, por ser $f$ continua, $Gr(f)\cong \mathbb{R}^+$, luego $Gr(f)$ es conexo. Como la adherencia de un conjunto conexo es conexa, tenemos que $C$ es conexo. Por un razonamiento análogo al visto en el ejemplo de la página~\pageref{ej:conexo_no_arcoconexo}, vemos que $C$ no es arcoconexo, ya que si lo fuera, podríamos unir un punto de $\{0\}\times [-1,1]$ con un punto de $Gr(f)$ mediante un arco, y por continuidad del arco, tendríamos que el límite del arco al acercarnos a $0$ sería un punto de la gráfica de $f$, lo cual es imposible (puesto que la gráfica de $f$ no tiene límite al acercarnos a $0$). Por lo que $C$ es un conjunto cerrado y conexo de $\mathbb{R}^2$ que no es arcoconexo.
   
   
   \begin{comment}
   Veamos sin embargo que $C$ no es arcoconexo.\\

   Supongamos que $C$ es arcoconexo. Consideramos cualquier punto $p\in \{0\}\times [-1,1]$ y $q\in Gr(f)$; por ejemplo $p=(0,0)$ y $q=(1, \sen(1))$. Como $C$ es arcoconexo, existe un arco $\alpha:[0,1]\to C$ que une $p$ con $q$. Definimos:
    \begin{equation*}
        t_0 = \inf\{t\in [0,1] : \alpha(t) \in Gr(f)\}
    \end{equation*}

    Definiendo $\alpha=(\alpha_x, \alpha_y)$, tenemos que:
    \begin{equation*}
        t_0 = \inf\{t\in [0,1] : \alpha_x(t) > 0\}
    \end{equation*}

    Notemos que $\alpha(t_0)\in \{0\}\times [-1,1]$.
   
   
   Por ser $\alpha$ continua, tenemos que:
   \begin{equation*}
       \lim_{t\to t_0^+} \alpha(t) = \alpha(t_0) = p = (0,0)
    \end{equation*}

    Considerando ahora $\alpha=(\alpha_x, \alpha_y)$, por ser las proyecciones continuas, tenemos que:
    \begin{equation*}
        \lim_{t\to 0^+} \alpha_x(t) = \alpha_x(0) = 0, \qquad \lim_{t\to 0^+} \alpha_y(t) = \alpha_y(0) = 0
    \end{equation*}
   \end{comment}
...
\begin{ejercicio}
    ...
    \noindent
    Sea $X$ un e.t., $x_0\in X$ y $C$ la componente arcoconexa de $x_0$ en $X$. En particular tenemos que $C$ es un conjunto arcoconexo, luego es conexo, por lo que está contenida en la componente conexa de $x$, al ser esta el mayor conjunto conexo que contiene a $x$.
\end{ejercicio}

\begin{ejercicio}
    En $\mathbb{R}$ con la topología de Sorgenfrey, esto es, la topología que tiene como base
    \begin{equation*}
        \cc{B}_S = \{\left[a,b\right[ \subset \mathbb{R} : a<b\},
    \end{equation*}
    ...
    \begin{equation*}
        U = \left[-\infty,y\right[\cap A, \qquad V = \left[y,+\infty\right[\cap A
    \end{equation*}
...
\begin{ejercicio}\label{ej:0.7}
    ...
    \begin{itemize}
        ...
        \item Porque existe $B\subset Y$ un conjunto arcoconexo distinto de $f(A)$ de forma que $f(A)\subset B\subset Y$. En dicho caso, si aplicamos $f^{-1}$ en la anterior inclusión (usando que se trata de una biyección), tenemos que:
            \begin{equation*}
...
\begin{ejercicio}
    ...
    \noindent
    Sean $p_1=(x_1,y_1),p_2=(x_2,y_2)\in X$ (donde $y_1, y_2\in \{0,1\}$), vamos a tratar de crear un arco que una $p_1$ con $p_2$:
    \begin{itemize}
        \item Si $y_1=y_2$, entonces $\gamma:[0,1]\to X$ dada por:
            \begin{equation*}
                \gamma(t) = ((1-t)x_1 + tx_2, y_1) \qquad \forall t\in [0,1]
            \end{equation*}
            Es una aplicación continua, ya que si tomamos $B = \left]a,b\right[\times \{0,1\}\in \cc{B}$, tenemos:
            \begin{equation*}
                \gamma^{-1}(B) = \gamma^{-1}(\left]a,b\right[\times \{0,1\}) = \gamma^{-1}(\left]a,b\right[\times \{y_1\}) \text{\ abierto de\ } [0,1]
            \end{equation*}
            Ya que el conjunto $\left]a,b\right[\times \{y_1\}$ es un abierto para la topología usual inducida en $X$ y $\gamma$ es una aplicación continua para la topología usual.
        \item Si $p_1 = (0,0)$ y $p_2 = (0,1)$, entonces si tomamos $\gamma:[0,1]\to X$ dada por:
            \begin{equation*}
                \gamma(t) = \left\{\begin{array}{ll}
                        p_1 & \text{si\ } 0 \leq t \leq \nicefrac{1}{2}\\
                        p_2 & \text{si\ } \nicefrac{1}{2}<t \leq 1
                \end{array}\right.
            \end{equation*}
            ...
            \begin{equation*}
                ...
                \end{array}\right.
            \end{equation*}
        \item Una vez discutidos dichos casos, suponemos ahora que $p_1=(x,0)$ y $p_2 = (y,1)$ (en caso contrario, sustituimos los papeles de $p_1$ y $p_2$), en cuyo caso:
            \begin{itemize}
                \item Sabemos de la existencia de un arco $\gamma$ que une $p_1$ con $(0,0)$.
                \item Sabemos de la existencia de un arco $\tau$ que une $(0,0)$ con $(0,1)$.
                \item Sabemos de la existencia de un arco $\pi$ que une $p_2$ con $(0,1)$.
            \end{itemize}
            Si consideramos el arco $\gamma \ast (\tau \ast \tilde{\pi})$ obtenemos un arco que une $p_1$ con $p_2$.
    \end{itemize} 
    ...
    \begin{description}
        ...
        \item [Opción 2.] Otra forma sería suponer que son homeomorfos, con lo que existe un homeomorfismo $f:\mathbb{R}\to X$. Sea $p\in \mathbb{R}$, resulta entonces que $\mathbb{R}\setminus \{p\}$ es homeomorfo a $X\setminus \{f(p)\}$, pero:
            \begin{itemize}
                \item $\mathbb{R}\setminus\{p\}$ no es arcoconexo.
                \item $X\setminus \{f(p)\}$ sí es arcoconexo, ya que podemos hacer que cualquier curva ``salte'' entre los dos conjuntos sin perder su continuidad, con lo que podemos seguir conectando dos puntos cualesquiera.
                % // TODO: Formalizar la prueba de que $X\setminus \{f(p)\}$ es arcoconexo.
            \end{itemize}
    \end{description}
\end{ejercicio}
...
\begin{ejercicio}
    ...
    \end{equation*}
    Puesto que $f$ es continua, obtenemos (como vimos en Topología I) un homeomorfismo entre $\Gamma$ y $X$. Como $\Gamma$ tiene 4 componentes arcoconexas:
    \begin{equation*}
        \mathbb{R}^+\times \mathbb{R}^+, \qquad \mathbb{R}^+\times \mathbb{R}^-, \qquad \mathbb{R}^-\times\mathbb{R}^-, \qquad \mathbb{R}^-\times\mathbb{R}^-
    \end{equation*}
    % // TODO: Formalizar que son las únicas componentes arcoconexas de $\Gamma$.
    y las componentes arcoconexas se convervan por homeomorfismos tal y como acabamos de ver en el ejercicio~\ref{ej:0.7}, tenemos que:
    \begin{equation*}
...
\begin{ejercicio} % // TODO: HACER
    ...
    Representamos en primer lugar el conjunto $X$ en la Figura~\ref{fig:conjunto_X}.
    \begin{figure}
        \centering
        \begin{tikzpicture}
            \begin{axis}[
                ...
                xmin=-3.5, xmax=3.5,
                ymin=-3.5, ymax=3.5,
                ticks=none, % Oculta marcas numéricas para centrar la atención en la topología
                legend style={
                    at={(1.02, 0.98)},
                    anchor=north west,
                    draw=black!20,
                ...
                % Entradas de leyenda virtuales para no duplicar entradas en el bucle
                \addlegendimage{thick, red}
                \addlegendentry{$A_n \equiv y = \nicefrac{1}{n}$}
                \addlegendimage{thick, blue}
                \addlegendentry{$B_n \equiv y = \nicefrac{-1}{n}$}

                % Dibujar las rectas horizontales A_n y B_n (forget plot evita que entren a la leyenda una a una)
                \foreach \n in {1,2,3,4,5} {
                    \addplot[thick, red, domain=-3:3, samples=2, forget plot] {1/\n};
                    \addplot[thick, blue, domain=-3:3, samples=2, forget plot] {-1/\n};
                }

                \addlegendimage{thick, olive}
                \addlegendentry{$C \equiv x = 0, y\neq 0$}

                % Eje de ordenadas vertical (x = 0) quitando el origen C
                \draw[thick, olive] (axis cs:0, -3.2) -- (axis cs:0, -0.08);
                \draw[thick, olive] (axis cs:0, 0.08) -- (axis cs:0, 3.2);

                % Origen no incluido (círculo hueco en el origen para indicar que falta)
                \draw[thick, black, fill=white] (axis cs:0, 0) circle (2pt);

                % Punto (1,0) marcado
                \node[circle, fill=black, inner sep=1.5pt] at (axis cs:1, 0) {};
            \end{axis}
        \end{tikzpicture}
        \caption{Representación del conjunto $X$.}
        \label{fig:conjunto_X}
    \end{figure}

    Veamos en primer lugar que $X$ es conexo. Definimos:
    \begin{align*}
        C^+ &= \{0\}\times \mathbb{R}^+ \qquad \text{y} \qquad C^- = \{0\}\times \mathbb{R}^-
    \end{align*}

    En primer lugar, $\{A_n\}_{n\in \mathbb{N}}\cup \{B_n\}_{n\in \mathbb{N}}\cup \{C^+,C^-\}$ es una familia de conjuntos conexos por ser todos ellos homeomorfos a $\mathbb{R}$. Por otro lado, tenemos que, para cada $n\in \mathbb{N}$, $(0,\nicefrac{1}{n})\in A_n \cap C^+$ y $(0,\nicefrac{-1}{n})\in B_n \cap C^-$. Por lo que $A_n\cap C^+ \neq \emptyset$ y $B_n\cap C^- \neq \emptyset$. Por tanto:
    \begin{itemize}
        \item $X_1 = \bigcup\limits_{n\in \mathbb{N}}A_n \cup C^+$ es un conjunto conexo, al intersectar todos los $A_n$ con $C^+$.
        \item $X_2 = \bigcup\limits_{n\in \mathbb{N}}B_n \cup C^-$ es un conjunto conexo, al intersectar todos los $B_n$ con $C^-$.
    \end{itemize}

    Consideramos ahora las respectivas adherencias de $X_1$ y $X_2$:
    \begin{align*}
        \ol{X_1} &= \left(\bigcup_{n\in \mathbb{N}}A_n\right) \cup C^+ \cup \left(\bb{R}\times \{0\}\right) \\
        \ol{X_2} &= \left(\bigcup_{n\in \bb{N}}B_n\right) \cup C^- \cup \left(\bb{R}\times \{0\}\right)
    \end{align*}

    Como $X_1$ es conexo y $X_1\subset X_1\cup \{(1,0)\} \subset \ol{X_1}$, tenemos que $X_1\cup \{(1,0)\}$ es conexo. De igual forma, $X_2\cup \{(1,0)\}$ es conexo. Puesto que la intersección de ambos conjuntos es no vacía, ya que $(1,0)\in X_1\cup \{(1,0)\}\cap X_2\cup \{(1,0)\}$, tenemos que:
    \begin{equation*}
        X_1\cup X_2 \cup \{(1,0)\} = \left(\bigcup_{n\in \mathbb{N}}A_n\right) \cup \left(\bigcup_{n\in \mathbb{N}}B_n\right) \cup C^+ \cup C^- \cup \{(1,0)\} = X
    \end{equation*}
    es un conjunto conexo, ya que es la unión de dos conjuntos conexos con intersección no vacía. Por tanto, el número de componentes conexas de $X$ es 1. Veamos no obstante que el número de componentes arcoconexas es 3.\\

    Por un razonamiento análogo al anterior, tenemos que $X_1$ y $X_2$ son conjuntos arcoconexos. Asimismo, el conjunto unitario $\{(1,0)\}$ es arcoconexo. Por lo que tenemos tres conjuntos arcoconexos que forman una partición de $X$: $X_1$, $X_2$ y $\{(1,0)\}$. Por un razonamiento análogo al visto en el ejemplo de la página~\pageref{ej:conexo_no_arcoconexo}, tenemos que $X_1$, $X_2$ y $\{(1,0)\}$ son las componentes arcoconexas de $X$, ya que si algún arco uniera un punto de dos de ellos, este no sería continuo, por lo que no existiría tal arco. Por tanto, el número de componentes arcoconexas de $X$ es 3.
    % // TODO: Formalizar el argumento de que no hay arcos que unan los tres conjuntos.
\end{ejercicio}
