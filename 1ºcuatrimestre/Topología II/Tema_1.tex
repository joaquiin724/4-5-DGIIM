...
\begin{definicion}
    ...
    \end{figure}
    \noindent
    A la aplicación $H$ la llamaremos \tbf{homotopía de $f$ en $g$}.

\end{definicion} 

\begin{observacion}
    Es importante notar, que  las imágenes pueden intersecarse. 
\end{observacion}

\begin{definicion}
    Dados $X$ un e.t., $x,y\in X$ y dos arcos $\alpha, \beta\in \Omega(X;x,y)$, decimos que $\alpha, \beta$ son \textbf{homotópicos por arcos} si existe $H:[0,1]\times [0,1] \to X$ continua tal que 
    \begin{align*}
    ...
    \end{align*}
    A $H$ se le llama \tbf{homotopía por arcos}.
\end{definicion}

\begin{lema}
    Sean $x,y$ dos puntos de un e.t. $X$, entonces el ser homotópicos por arcos es una relación de equivalencia en $\Omega(X;x,y)$.
    ...
    \begin{proof}\
        \begin{enumerate}
            \item[(i)] Dado $\alpha \in \Omega(X;x,y)$ queremos ver que $\alpha$ es homotópica por arcos con $\alpha$. Para ello podemos definir la homotopía por arcos
            \Func{H}{[0,1]\times[0,1]}{X}{(s,t)}{\alpha(s)}
            que es claramente continua. Además, como $\alpha\in \Omega(X;x,y)$, tenemos que $\alpha(s)\in X$ para todo $s\in [0,1]$. Además se verifica que
            \begin{gather*}
                H(s,0) = \alpha(s)=H(s,1)\\
                H(0,t) = \alpha(0)=x \hspace{1cm} H(1,t) = \alpha(1) = y
            \end{gather*}

            \item[(ii)] Dados $\alpha, \beta\in \Omega(X;x,y)$ tales que existe $H:[0,1]\times [0,1]\to X$ continua verificando
            \begin{align*}
            ...
            \end{align*}
            es decir que $\beta$ es homotópica por arcos a $\alpha$. Tomando $\tilde{H}(s,t):=H(s, 1-t)$ cumple claramente con lo que buscamos:
            \begin{gather*}
                \tilde{H}(s,0) = H(s, 1-0) = H(s,1) = \beta(x)\\
                \tilde{H}(s, 1) = H(s, 1-1) = H(s,0) = \alpha(x)\\
                \tilde{H}(0,t) = H(0, 1-t) = x\\
                \tilde{H}(1,t) = H(1, 1-t) = y
            \end{gather*}
            ...
            \end{align*}
            es decir, que $\alpha$ es homotópica por arcos a $\gamma$. Para ello consideramos 
            \begin{gather*}
                H(s,t) = \left\{
                    \begin{array}{l c c}
...
\begin{observacion}
    Este resultado es análogo para homotopías. Es decir, ser homotópico da lugar a una relación de equivalencia en $C(X,Y) = \{f :X\to Y : f \text{ continua}\}$.
\end{observacion}

\begin{notacion}
    Por convenio, dado un arco cualquiera $\alpha\in \Omega(X;x,y)$ denotaremos por $[\alpha]$ a la clase de equivalencia de $\alpha$ bajo la relación de ser homotópicos por arcos.
\end{notacion}

\begin{ejemplo}\
    \begin{enumerate}
        \item Sean $X$ un e.t. y $f,g:X \to \bb{R}^n$ aplicaciones continuas entonces se tiene que $f$ y $g$ son homotópicas.
        \begin{proof}
            Vamos a definir la aplicación
            \Func{H}{X \times [0,1]}{\bb{R}^n}{(x,t)}{(1-t)f(x) + tg(x)}
        ...
        \end{proof}
        En el caso particular de que $f=\alpha$ y $g=\beta$ fuesen arcos comenzando en un punto común y acabando en otro punto común, entonces la $H$ anterior (que fija los extremos) sería una homotopía por arcos, y tendríamos $[\alpha]=[\beta]$.

        \item Intuitivamente hay arcos en $\R^2\setminus\{(0,0)\}$ que no son homotópicos entre arcos. Este hecho no lo hemos demostrado todavía y no lo haremos ahora, pero es una consecuencia intuitiva de la definición.

        \item Si $\alpha: [0,1]\to X$ es un arco en $\Omega(X;x,y)$ y $h:[0,1] \to [0,1]$ es una aplicación continua con $h(0)=0$ y $h(1)=1$, entonces el arco
        \begin{equation*}
            \dot{\alpha}(s) = \alpha(h(s))
        \end{equation*}
        cumple $[\alpha] = [\dot{\alpha}]$, es decir, son homotópicos por arcos.
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                H(0,t) = \alpha(0)=\dot{\alpha}(0)=0 \hspace{1cm} & H(1,t) = \alpha(1) = \dot{\alpha}(1)=1
            \end{align*}
        \end{proof}
...
\begin{lema}
    Dados dos arcos $\alpha_1, \alpha_2$ en $\Omega(X;x,y)$ y $\beta_1, \beta_2\in \Omega(X; y,z)$. Se verifica que si $[\alpha_1] = [\alpha_2]$ y $[\beta_1] = [\beta_2]$, entonces 
    $$[\alpha_1\ast\beta_1] = [\alpha_2\ast\beta_2]$$
    ...
    \begin{proof}
        ...
        \begin{gather*}
            \begin{array}{c c c c}
                ...
                H_1(s,1) = \alpha_2(s) &&& H_2(s,1) = \beta_2(s)\\
                H_1(0,t) = x &&& H_2(0,t) = y\\
                H_1(1, t) = y &&& H_2(1,t) = z
            \end{array}
        \end{gather*}
...
\begin{definicion}
    A partir del lema anterior, podemos definir la \tbf{operación} $\ast$ \tbf{en las clases de equivalencia} de la siguiente manera:
    \begin{gather*}
...
\begin{teo}
    Dado $X$ un e.t., $\alpha\in \Omega(X;x,y)$, $\beta\in \Omega(X;y,z)$ y $\gamma\in \Omega(X;z,w)$ se tiene que 
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \begin{figure}[H]
                ...
                \begin{tikzpicture}
                    ...
                    \draw[green!60!black, thick] plot[smooth, tension=1] coordinates {
                    ...
                    % Leyenda colores
                    \draw[violet, thick] (13,2)--(13.5,2);
                    \draw[orange!90!black, thick] (13,0)--(13.5,0);
            ...
            \item[(iii)] Tendremos que ver en este caso que $\displaystyle \alpha\ast\tilde{\alpha}$ es homotópico por arcos a $\veps_x$. Describamos ambas curvas.
            \begin{gather*}
...
\begin{teo}
    ...
    \begin{proof}
        ...
        \Func{F_\alpha}{\pi_1(X,y)}{\pi_1(X,x)}{[\gamma]}{[\alpha]\ast[\gamma]\ast[\tilde{\alpha}]}
...
\begin{lema}
    ...
    \Func{(f_x)_*}{\pi_1(X,x)}{\pi_1(Y,f(x))}{[\alpha]}{[f\circ \alpha]}
...
\begin{propiedades}
    ...
    \begin{enumerate}
        ...
        \Func{Id_*}{\pi_1(X,x_0)}{\pi_1(X,x_0)}{[\alpha]}{[\alpha]}
...
\begin{teo}
    ...
    \begin{proof}
        Veamos que 
        \Func{\phi}{\pi_1(X\times Y,(x,y))}{\pi_1(X,x) \times \pi_1(Y,y)}{[\alpha]}{([p_1 \circ \alpha], [p_2\circ \alpha])}
        ...
        \Func{\psi}{\pi_1(X,x)\times \pi_1(Y,y)}{\pi_1(X\times Y, (x,y))}{([\beta], [\gamma])}{[(\beta, \gamma)]}
...
\begin{ejemplo} 
    En este caso, si consideramos la aplicación $p:\bb{R} \to \bb{S}^1$ descrita en la gráfica, que es claramente continua y sobreyectiva\footnote{De hecho, es un homeomorfismo local en cada intervalo con amplitud 1 que estudiamos en Topología I}. Podemos ver que $O$, que contiene al $(1,0)$ está regularmente recubierto
...
\begin{teo}
    ...
    \begin{proof}
        ...
        \begin{gather*}
            p':\left[k-\frac{1}{4}, k+\frac{1}{4}\right] \to \overline{O}
        \end{gather*}
        es un homeomorfismo y por tanto su restricción en el dominio
        \begin{gather*}
            p':\left(k-\frac{1}{4}, k+\frac{1}{4}\right) \to \overline{O}
        \end{gather*}
        también lo es y por la definición de $A_k$ tenemos lo que buscábamos. \\
...
\begin{ejemplo}
    La aplicación
    \Func{p}{\bb{S}^1}{\bb{S}^1}{(x,y)}{(x^2-y^2, 2xy)}
    ...
    \Func{p_n}{\bb{S}^1}{\bb{S}^1}{(\cos(\theta), \sen(\theta))}{(\cos (n\theta), \sen (n\theta))}
    o equivalentemente
    \Func{p_n}{\bb{S}^1}{\bb{S}^1}{(x+iy)}{(x+iy)^n}
...
\begin{propiedades}\
    \begin{enumerate}
        ...
        \Func{p_1\times p_2}{R_1\times R_2}{B_1\times B_2}{(x,y)}{(p_1(x), p_2(x))}
...
\begin{ejemplo}\
    \begin{enumerate}
        \item $\bb{R}^2$ es un recubridor del cilindro $\bb{S}^1\times \bb{R}$. Para verlo podemos considerar la aplicación dada por
        \Func{p_1}{\bb{R}}{\bb{S}^1}{x}{(\cos(2\pi x), \sen(2\pi x))}
        y la identidad dada por
        \Func{p_2=Id_{\bb{R}}}{\bb{R}}{\bb{R}}{x}{x}
        Como $p_1$ es un recubrimiento y $p_2$ es un homeomorfismo tendremos que la aplicación
        \Func{p_1\times p_2}{\bb{R}^2}{\bb{S}^1\times \bb{R}}{(x,y)}{(\cos(2\pi x), \sen(2\pi x), y)}
        ...
        \Func{p}{\bb{R}}{\bb{S}^1}{x}{(\cos(2\pi x), \sen(2\pi x))}
         y su producto 
         \Func{p\times p}{\bb{R}\times \bb{R}}{\bb{S}^1\times \bb{S}^1}{(x,y)}{(\cos(2\pi x), \sen(2\pi x), \cos(2\pi y), \sen(2\pi y))}
        ...
        \item Intuitivamente podemos pensar en un recubrimiento de una circunferencia tangente a una recta como una colección de rectas paralelas y atravesadas por una común, como aparece en la figura. De esta forma (tal cual está en el dibujo) al ``enrollar'' el eje $x$ de forma que hagamos coincidir todas las rectas rojas tendremos que la recta azul forma una circunferencia, que como originalmente solo tocaba en un punto a cada recta, resultará en la circunferencia tangente a la recta.
...
% //TODO: mirar si se queda igual que con gather
...
\begin{definicion}
    ...
    \begin{tikzcd}
        & R \arrow[d, "p"] \\
        X \arrow[ur, dashed, "\hat{f}"] \arrow[r, "f"'] & B
    \end{tikzcd}
    \]
\end{definicion}
...
\begin{ejemplo}
    ...
    \Func{p}{\bb{R}}{\bb{S}^1}{x}{(\cos(2\pi x), \sen(2\pi x))}
    y tomamos
    \Func{\alpha}{[0,1]}{\bb{S}^1}{s}{(\cos(2\pi s), \sen(2\pi s))}
    Si elegimos
    \Func{\hat{\alpha}}{[0,1]}{\bb{R}}{s}{s}
    es claro que $p\circ \hat{\alpha} = \alpha$. Otros posibles levantamientos de $\alpha$ son 
    \Func{\hat{\alpha}_k}{[0,1]}{\bb{R}}{s}{k+s}
    con $k\in \bb{Z}$.\\
...
\begin{definicion}
    ...
    \Func{\phi}{\pi_1(B,b_0)}{p^{-1}(b_0)}{[\alpha]}{\hat{\alpha}(1)}
...
\begin{ejemplo}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            % Abiertos en preimagen
            \draw[line width=1pt, green!60!black, ->] (-2, 2.96) -- (2, 2.96);
            \draw[line width=1pt, blue, ->] (-2, 3) -- (0, 3);
            \draw[line width=1pt, red, ->] (-2, 3.04) -- (-1.75, 3.04);
            ...
            \draw[thick] (0,-1) circle [radius=1.5];
            \node[anchor=west] at (1.7,0) {$B=\bb{S}^1$};
            \draw[green!60!black, line width=1pt] (1.54,-1) arc (0:180:1.54);
            \draw[green!60!black, line width=1pt] (1.6,-1) arc (0:-180:1.57);
            \draw[green!60!black, line width=1pt] (1.6,-1) arc (0:180:1.6);
            \draw[green!60!black, line width=1pt, <-] (1.66,-1) arc (0:-180:1.63);
...
\begin{teo}
    ...
    \begin{proof}
        ...
        Como coinciden los elementos del grupo fundamental tendremos que son homotópicos por arcos por lo que existe una homotopía por arcos entre $\hat{\alpha}$ y $\hat{\beta}$, es decir, una aplicación $H:[0,1]\times [0,1] \to R$ tal que 
        \begin{gather*}
            H(t,0) = \hat{\alpha}(t) \ \ \ \text{ y } \ \ \ H(t, 1) = \hat{\beta}(t)
        \end{gather*}
        Si consideramos ahora su composición con la aplicación recubridora, $p\circ H$, tendremos que es una homotopía entre $\alpha$ y $\beta$ ya que tendríamos $p\circ H : [0,1] \times [0,1] \to B$ que verifica
        \begin{gather*}
            (p\circ H)(t,0) = p(H(t,0)) = p(\hat{\alpha}(t)) = \alpha(t)\\
            (p\circ H)(t,1) = p(H(t,1)) = p(\hat{\beta}(t)) = \beta(t)
        \end{gather*}
        por lo que podemos concluir que $[\alpha] = [\beta]$.
    \end{proof}
\end{teo}
...
\begin{definicion}
    Dado un lazo $\alpha:[0,1]\to \bb{S}^1 \subseteq \bb{R}^2$ basado en $(1,0)$, definimos el \textbf{grado} de $\alpha$ como el único número entero $\hat{\alpha}(1)$ dado por el levantamiento $\hat{\alpha}$ de $\alpha$ mediante la aplicación recubridora
    \begin{gather*}
        p(x) = (\cos(2\pi x), \sen(2\pi x))
    \end{gather*}
    con $\hat{\alpha}(0) = 0$. Notaremos al grado de $\alpha$ como $\deg(\alpha)$.
\end{definicion}
...
\begin{teo}
    ...
    \Func{\deg}{\pi_1(\bb{S}^1, (1,0))}{\bb{Z}}{[\alpha]}{\deg(\alpha)}
    ...
    \begin{proof}
        ...
        \end{align*}
        por lo que $\widehat{\alpha\ast\beta}$ es un levantamiento de $\alpha \ast \beta$ con $\widehat{\alpha \ast \beta}(0) = \widehat{\alpha}(0)$ por lo que por definición tenemos que 
        \begin{gather*}
...
\begin{prop}\label{prop1-12}
    ...
    \begin{proof}
        ...
        \end{gather*}
        ya que $\overline{\bb{D}}$ es simplemente conexo. Si existiese una $f$ continua, entonces tendríamos que
        \Func{f_*}{\pi_1(\overline{\bb{D}}(1,0))}{\pi_1(\bb{S}^1, (1,0))}{[\alpha]_{\overline{\bb{D}}}}{f_*([\alpha]_{\overline{\bb{D}}}) \overset{def}{=}[f\circ \alpha]_{\bb{S}^1} = [\alpha]_{\bb{S}^1}}
        es un homomorfismo. Pero $[\alpha]_{\overline{\bb{D}}} = [\veps_{(1,0)}]_{\overline{\bb{D}}}$ por lo que 
        \begin{gather*}
            f_*([\alpha]_{\overline{\bb{D}}}) = f_*([\veps_{(1,0)}]_{\overline{\bb{D}}}) = [\veps_{(1,0)}]_{\bb{S}^1}
        \end{gather*}
        Sin embargo, teníamos que $f_*([\alpha]_{\overline{\bb{D}}}) = [\alpha]_{\bb{S}^1}$ y llegamos a contradicción ya que $\deg(\alpha) = 1$ y $\deg(\veps_{(1,0)}) = 0$.
    \end{proof}
\end{prop}
...
\begin{observacion}
    ...
    \begin{proof}
        ...
        \end{gather*}
        Entonces tendríamos $h^{-1}\circ f \circ h : \overline{\bb{D}} \to \overline{\bb{D}}$ continua y por el teorema de Brouwer existe $y_0\in \overline{\bb{D}}$ tal que $(h^{-1}\circ f \circ h)(y_0) = y_0$.
    \end{proof}
\end{observacion}
...
\begin{coro}
    ...
    \begin{proof}
        Consideramos la aplicación $f(x) = \frac{V(x)}{|V(x)|}$ que es continua (ya que $V(x)\neq 0$ para todo $x\in \overline{{\bb{D}}}$). Por el teorema de Brouwer tenemos que $\exists x_0\in \overline{\bb{D}}$ tal que $f(x_0) = x_0$ por lo que 
        \begin{gather*}
            x_0 = \frac{V(x_0)}{|V(x_0)|} \Rightarrow V(x_0) = |V(x_0)|x_0
        \end{gather*}
        
        Tomando $\lambda = |V(x_0)|$ tenemos lo que buscábamos. Si tomamos ahora $g(x) = - \frac{V(x_0)}{|V(x_0)|}$ tendríamos análogamente que existe un $y_0$ tal que $y_0 = g(y_0) = \frac{V(y_0)}{|V(y_0)|}\in \bb{S}^1$ y tendríamos
        \begin{gather*}
            V(y_0) = -|V(y_0)|y_0
        \end{gather*}
        Tomando $\mu = |V(y_0)|$ se verifica la segunda parte del corolario. En general, podemos considerar cualquier ángulo $\theta$ y definir la aplicación como
        \begin{gather*}
...
\begin{teo}[Teorema fundamental del álgebra]
    ...
    \begin{proof}
        ...
        \end{gather*}
        Esto es fácil de ver ya que tenemos
        \begin{gather*}
        ...
        \begin{align*}
            H(0,t) = 1\ \ \ \ \ & \forall t\in [0,1]\\
            H(1,t) = 1\ \ \ \ \  & \forall t\in [0,1]\\
            H(s,0) = 1\ \ \ \ \  & \forall s\in [0,1]\\
            H(s,1) = e^{2\pi i n s} \ \ \ \ \ & \forall s\in [0,1]
        \end{align*}
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \Func{r}{\bb{R}^{n+1}\setminus\{0\}}{\bb{S}^1}{x}{\frac{x}{|x|}}
        que es claramente continua y se verifica que $r(x) = x$ para todo $x\in \bb{S}^1$.

        \item $\bb{S}^1$ no es un retracto de $\overline{\bb{D}}$ por lo visto en la proposición \ref{prop1-12}.
...
%         \begin{gather*}
%             \xymatrixrowsep{0.5pc}\xymatrixcolsep{1.5pc}\xymatrix{
%                 A \ar[r]^{i} & X \ar[r]^{r} & A & ,r\circ i = Id_A\\
%                 a \ar@{|->}[r] & a \ar@{|->}[r] & a
%             }
%         \end{gather*}
...
\begin{lema}
    ...
    \begin{proof}
        ...
        \begin{tikzcd}[row sep=tiny, column sep=small]
            A \arrow[r, "i"] & X \arrow[r, "r"] & A & \text{con } r\circ i = \mathrm{Id}_A \\
            a \arrow[r, maps to] & a \arrow[r, maps to] & a
        \end{tikzcd}
        \]

        \[
        \begin{tikzcd}
            \pi_1(A,a_0) \arrow[r, "i_*"] \arrow[rr, bend right=25, "r_* \circ i_*"'] &
            \pi_1(X,a_0) \arrow[r, "r_*"] &
            \pi_1(A,a_0)
        \end{tikzcd}
        \]

        \[
            r_* \circ i_* = (\mathrm{Id}_A)_* = \mathrm{Id}_{\pi_1(A,a_0)}
        \]
            
        Como $\mathrm{Id}_{\pi_1(A,a_0)}$ es inyectiva, entonces se tiene que $i_*$ es inyectiva.
    \end{proof}
\end{lema}
...
\begin{observacion}\
    \begin{enumerate}
        \item La inversa homotópica no tiene por qué ser única
        \item La composición de equivalencias homotópicas sigue siendo una equivalencia homotópica, de lo que se deduce que ser homotópicamente equivalente es una relación de equivalencia.
        \item Una equivalencia homotópica no tiene por qué ser ni inyectiva ni sobreyectiva. 
        \item Todo homeomorfismo es una equivalencia homotópica.
    \end{enumerate}
\end{observacion}
...
\begin{lema}
    ...
    \end{gather*}
    donde $\varphi([\gamma]) = [\tilde{\alpha}] \ast [\gamma] \ast [\alpha]$. En particular, $f_*$ es inyectiva (respectivamente sobreyectiva) si y solo si $g_*$ lo es.
    ...
    \begin{proof}
        ...
        \Func{F}{[0,1]\times[0,1]}{X\times [0,1]}{(s,t)}{(\beta(s), t)}
        que es claramente continua y los siguientes lazos en $[0,1]\times [0,1]$
        \begin{align*}
            b_0(s) = (s,0)\ \ \ \ \ & ,s\in [0,1]\\
            b_1(s) = (s,1)\ \ \ \ \  & ,s\in [0,1]\\
            h_0(s) = (0,s)\ \ \ \ \  & ,s\in [0,1]\\
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                % Curva
                \draw[blue, thick] plot[smooth cycle, tension=1] coordinates {
                    (3,-0.5) (3.2,-0.2) (3.6,0.1) (4.2,0.5) (4.4,0.3) (4.1,-0.2)
                };
                \draw[dashed, thick] plot[smooth cycle, tension=1] coordinates {
                    (3,0) (3.2,0.3) (3.6,0.6) (4.2,1) (4.4,0.8) (4.1,0.1)
                };
                \draw[red, thick] plot[smooth cycle, tension=1] coordinates {
        ...
        Como $[0,1]\times [0,1]$ es simplemente conexo y $b_0\ast h_1 \ast \tilde{b_1} \ast \tilde{h_0}$ es un lazo basado en el $(0,0)$, entonces se tiene que 
        \begin{gather*}
        ...
        \begin{align*}
            ...
            &= \left\{
                ...
                \end{array}
            \right\} = ((f\circ \beta) \ast \alpha)(s)
        \end{align*}

        \begin{align*}
            (H\circ F \circ G) (s,1) &= H(F((h_0\ast b_1)(s))) = (H\circ F)\left(\left\{
                \begin{array}{l c c}
                    h_0(2s) & \text{ si } & s\in [0, \nicefrac{1}{2}]\\
                    b_1(2s-1) & \text{ si } & s \in [\nicefrac{1}{2}, 1]
                \end{array}
            \right\}\right) = \\
            &= (H\circ F) \left(\left\{
                \begin{array}{l c c}
            ...
            &= H \left(\left\{
                \begin{array}{l c c}
                    (x_0, 2s) & \text{ si } & s\in [0, \nicefrac{1}{2}]\\
                    (\beta(2s-1), 1) & \text{ si } & s \in [\nicefrac{1}{2}, 1]
                \end{array}
            \right\}\right) = \\
            &= \left\{
                \begin{array}{l c c}
                    \alpha(2s) & \text{ si } & s\in [0, \nicefrac{1}{2}]\\
                    g(\beta(2s-1)) & \text{ si } & s \in [\nicefrac{1}{2}, 1]
                \end{array}
            \right\} = (\alpha \ast (g\circ \beta))(s)
        \end{align*}
...
\begin{teo}
    ...
    \begin{proof}
        ...
        \begin{gather*}
            \xymatrix{ %
            \pi_1(X,x_0) \ar[r]^{f_*} \ar[drr]_{\overset{\scriptstyle(Id_X)_*}{\text{isom.}}} & \pi_1(Y,f(x_0)) \ar[r]^{g_*} & \pi_1(X,g(f(x_0))) \ar[d]^{\displaystyle \varphi \equiv \text{isomorfismo}}\\
            && \pi_1(X,x_0)
            \ar@{}[lu]|{//}
            }
        \end{gather*}
        De aquí se deduce que $f_*$ es inyectiva y que $g_*$ es sobreyectiva
        \begin{gather*}
            \xymatrix{ %
            \pi_1(Y,f(x_0)) \ar[r]^{g_*} \ar[drr]_{\overset{\scriptstyle(Id_Y)_*}{\text{isom.}}} & \pi_1(X,g(f(x_0))) \ar[r]^{f_*} & \pi_1(Y, f(g(f(x_0)))) \ar[d]^{\displaystyle \tilde{\varphi} \equiv \text{isomorfismo}}\\
            && \pi_1(X,x_0)
            \ar@{}[lu]|{//}
            }
        \end{gather*}
        De este segundo diagrama concluimos que $g_*$ es inyectiva y que $f_*$ es sobreyectiva. En particular tenemos que $f_*$ es biyectiva. Como es el homomorfismo inducido de $f$ tendremos que $f_*$ es un isomorfismo.
    \end{proof}
\end{teo}

\begin{definicion}
    Sean $X$ un espacio topológico y $A\subset X$. Decimos que $A$ es un \textbf{retracto de deformación} de $X$ si existe una retracción $r:X\to A$ tal que $i\circ r$ sea homotópica a la identidad en $X$, donde $i: A \to X$ denota la inclusión.\\

    Esto es equivalente a que exista una homotopía $H:X\times[0,1]\to X$ tal que 
    \begin{gather*}
        H(x,0) = x \ \ \ \forall x \in X\\
        H(x,1) \in A \ \ \ \forall x \in X\\
        H(a,1) = a \ \ \ \forall a\in A
    \end{gather*}
    donde la retracción será $r(x)=H(x,1)$.
\end{definicion}
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        Veamos que $\bb{S}^1$ es retracto de deformación de $X$. Para ello buscamos la homotopía $H:X\times[0,1]\to X$ y la definimos de la siguiente forma:
        \begin{gather*}
            H(x,t) = \left\{
                \begin{array}{l c l}
...
\begin{coro}
    ...
    \end{gather*}
    \begin{proof}
        Si $A$ es un retracto de deformación de $X$, entonces existe una homotopía $H:X\times[0,1]\to X$ cumpliendo
        \begin{gather*}
            H(x,0) = x \ \ \ \forall x \in X\\
            H(x,1) \in A \ \ \ \forall x \in X\\
            H(a,1) = a \ \ \ \forall a\in A
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \end{align*}
        sería biyectiva y tendríamos $(\bb{Z}, +) \cong \{0\}$ lo cual es absurdo.

        \item $\bb{S}^1 \times \{0\}$ es un retracto de deformación de $\bb{S}^1 \times \bb{R}$. Si consideramos la aplicación
        \Func{H}{(\bb{S}^1 \times \bb{R}) \times [0,1]}{\bb{S}^1 \times \bb{R}}{((x_1,x_2,x_3),t)}{(x_1,x_2, (1-t)x_3) = (1-t)(x_1,x_2,x_3) + t(x_1,x_2,0)}
        ...
        \Func{H}{Y \times [0,1]}{Y}{((x,y),t)}{(1-t)(x,y)+t\left(x, \frac{1}{2}\right)}
        que es continua. Veamos que $H$ está bien definida, es decir, que para cualesquiera 2 elementos relacionados tienen la misma imagen (bajo $R$).\\
        
        Supongamos $(x_1, y_1),(x_2,y_2)\in Y$ tal que $(x_1,y_1)R(x_2,y_2)$. Si $(x_1,y_1)=(x_2,y_2)$ es claro que $H((x_1,y_1), t) = H((x_2,y_2), t)$. Supongamos entonces que $(x_1,y_1)\neq(x_2,y_2)$. Podemos suponer sin pérdida de generalidad que $x_1=0$, $x_2=1$ y además sabemos que $y_1+y_2 = 1$. Tendremos entonces
        \begin{align*}
            H((x_1,y_1), t) &= (1-t)(x_1,y_1)+t\left(x_1, \frac{1}{2}\right) = \\
            &= (1-t)(0,y_1)+t\left(0, \frac{1}{2}\right) = \left(0,(1-t) y_1 + \frac{1}{2}t\right)\\\\
            H((x_2,y_2), t) &= (1-t)(x_2,y_2)+t\left(x_2, \frac{1}{2}\right) = \\
            &= (1-t)(1,y_2)+t\left(1, \frac{1}{2}\right) = \left(1-t+t, (1-t)y_2 + \frac{1}{2}t\right) =\\
            &= \left(1, (1-t)y_2 + \frac{1}{2}t\right)
        \end{align*}
        Veamos entonces que $(H((x_1,y_1), t))R(H((x_2,y_2), t))$.Fijamos $t\in [0,1]$ y aplicamos la siguiente notación $H((x_1,y_1), t) = (x_1', y_1')$, $H((x_2,y_2), t) = (x_2', y_2')$. De esta forma tenemos que
        \begin{align*}
            \{x_1', x_2'\} &= \{0,1\}\\
            y_1'+y_2' &= (1-t) y_1 + \frac{1}{2}t + (1-t)y_2 + \frac{1}{2}t =\\
            &= (1-t)(y_1+y_2) + \left(\frac{1}{2} + \frac{1}{2}\right)t = 1-t + t = 1
        \end{align*}
        Como $t$ era arbitrario podemos concluir que la aplicación $H$ está bien definida. De esta forma se tiene
        \begin{gather*}
...
\begin{coro}
    ...
    \begin{proof}
        ...
        \begin{gather*}
            ...
            H(x,1) = x_0
        \end{gather*}
        Definimos para cada $x\in X$ la aplicación $\alpha_x(t)=H(x,t)$ es un arco que une $x$ con $x_0$ para todo por lo que $X$ es arcoconexo.\\
...
Parece intuitivo que el grupo fundamental de una esfera es el trivial ya que podríamos llevar cualquier lazo al trivial. Efectivamente la intuición no nos engaña pero tendremos que verlo de una forma más rigurosa para concluir la veracidad de esta afirmación.
...
\begin{coro}
    Las esferas $\bb{S}^n$ para $n\geq 2$ son simplemente conexas. En particular su grupo fundamental es el trivial.
...
\begin{coro}
    El grupo fundamental de $\bb{R}\bb{P}^n$ es isomorfo a $\bb{Z}_2$ para $n\geq 2$.
    ...
    \begin{proof}
        ...
        \begin{gather*}
            \varphi: \pi_1(\bb{R}\bb{P}^n, [a_0]) \to p^{-1}([a_0]) = \{a_0,-a_0\}
        \end{gather*}
        es biyectiva (ya que $\bb{S}^1$ es simplemente conexo). Entonces $\pi_1(\bb{R}\bb{P}^n, [a_0])$ solo tiene dos elementos, luego $\pi_1(\bb{R}\bb{P}^n, [a_0]) \cong \bb{Z}_2$.
    \end{proof}
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        Sea $\{x_0\} = S_1\cap S_2$ y tomamos $U=X\setminus\{y_1\}$ con $y_1\in S_1\setminus\{x_0\}$ y $V=X\setminus\{y_2\}$ con $y_2\in S_2\setminus\{x_0\}$. Es claro que $X=U \cup V$ y $U,V$ son abiertos. $U\cap V$ es arcoconexo por ser unión de dos arcoconexos con intersección no vacía $(S_1\setminus\{y_1\})\cup (S_2\setminus\{y_2\})$ ya que 
        \begin{gather*}
        ...
        \begin{gather*}
            \pi_1(U)\overset{\text{isom.}}{\cong}\pi_1(S_2)\cong \{0\}
        \end{gather*}
        Sabemos que $S_1\setminus\{y_1\}$ es homeomorfo a $\bb{R}^2$ y podemos suponer que la imagen mediante ese homeomorfismo de $x_0$ es el origen $(0,0)$. Entonces la composición de este homeomorfismo y la homotopía 
        \begin{gather*}
        ...
        \begin{align*}
            \tilde{H}:U \times [0,1] & \longrightarrow U\\
            (x,t) & \longmapsto \left\{
                \begin{array}{l c c}
            ...
            \right.
        \end{align*}
        es una homotopía desde la identidad hasta la que deja fijos a todos los puntos de $S_1$, luego $S_2$ es un retracto de deformación de $U$ y llegamos a que $\pi_1(U)\cong \{0\}$.\\

        Análogamente tenemos que $S_1$ es un retracto de deformación de $V$ por lo que $\pi_1(V) \cong \{0\}$.\\

        Tenemos entonces que $X=S_1\cup S_2$ es unión de dos conjuntos $U$ y $V$ simplemente conexos y con intersección arcoconexa. Por el teorema anterior llegamos a que $X$ es simplemente conexo, luego su grupo fundamental es el trivial
        \begin{gather*}
            \pi_1(R_1\cup R_2) = \{0\}
        \end{gather*}
...
\begin{lema}
    Sea $f:\bb{S}^1 \to \bb{S}^1$ una aplicación continua que conserva antípodas, es decir, $f(-x)=-f(x)$ para todo $x\in \bb{S}^1$. Si consideramos el lazo
    \begin{gather*}
...
\begin{teo}[de las tortitas]
    Sean $A_1$ y $A_2$ dos compactos de $\bb{R}^2$, entonces existe una única recta que divide a ambos a la vez dejando la mitad del área de cada uno a cada lado de la recta.
    ...
    \begin{proof}
        ...
        \Func{f}{\bb{S}^2}{\bb{R}^2}{x}{(area(A_1\cap P_x^+), area(A_2\cap P_x^+))}
...
\begin{observacion}
    ...
    \begin{enumerate}
        \item Cuando hay dos letras consecutivas del mismo grupo se multiplican usando la operación de ese grupo.
...
\begin{lema}[Propiedad universal del producto libre]
    Sea $\{G_i\}_{i\in I}$ una familia de grupos disjuntos\footnote{dos a dos}. Consideramos una familia de homomorfismos
    \[
        f_i: G_i \to G
    \]
    sobre un grupo $G$ fijo. Entonces existe un único homomorfismo
    \[
        f : \mathop{\ast}_{i\in I} G_i \to G
    \]
    tal que el siguiente diagrama conmuta:
    \[
    \begin{tikzcd}
        G_i \arrow[rr, "f_i"] \arrow[dr, "h_i"'] & & G \\
        & \displaystyle\mathop{\ast}_{i\in I} G_i \arrow[ur, dashed, "f"'] &
    \end{tikzcd}
    \]
    donde $h_{i_0}:G_{i_0}\to \mathop{\ast}_{i\in I} G_i$ es el homomorfismo inclusión. En la práctica se tendrá que 
    \[
        f(x_1,\dots,x_n) = f_{i_1}(x_1) \cdots f_{i_n}(x_n)
    \]
    donde $x_k \in G_{i_k}$ para todo $k\in \{1,\dots,n\}$.
\end{lema}
...
\begin{observacion}
    ...
    Podemos considerar $\rho = Id_G$ y aplicando el primer teorema de isomorfía sobre la $f$ que da la propiedad universal tenemos que 
    \begin{gather*}
        G \cong \frac{F(G)}{\ker(f)}
    \end{gather*}
    De esta forma se deduce que todo grupo $G$ puede verse como un grupo libre sobre un subgrupo normal suyo.
\end{observacion}
...
\begin{teo}[Seifert-van Kampen]
    ...
    \begin{gather*}
        R = \{i_{1*}(g)^{-1}i_{2*}(g) : g\in \pi_1(U\cap V,x_0)\}
    \end{gather*}
    Por tanto, 
    \begin{gather*}
    ...
    \begin{enumerate}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                \node[draw, circle, fill=red, red, text=red, inner sep=1pt, label={[red]left:{$x_0$}}] at (0,0) {};
        ...
        \begin{gather*}
            ...
            V = X \setminus \{x_2\}
        \end{gather*}
        y tenemos que $U$ es abierto y arcoconexo (ya que $C_2$ es arcoconexo, $C_1\setminus \{x_1\}$ es arcoconexo y se intersecan en un punto $x_0$) y de forma análoga $V$ también es abierto y arcoconexo. Es por esto que $U\cap V$ es arcoconexo (dos abiertos arcoconexos que se intersecan).\\

        Tenemos que $C_2$ es retracto de deformación de $U$ por lo que
        \begin{gather*}
            \pi_1(U,x_0) \overset{isom.}{\cong} \pi_1(C_2,x_0) \overset{isom.}{\cong} \bb{Z}
        \end{gather*}

        De la misma forma 
        \begin{gather*}
            \pi_1(V,x_0) \overset{isom.}{\cong} \pi_1(C_1,x_0) \overset{isom.}{\cong} \bb{Z}
        \end{gather*}
        ...
        \end{gather*}
        donde $i_{1*}([\gamma]_{U\cap V})^{-1} i_{2*}([\gamma]_{U\cap V})= [\gamma]_U^{-1}[\gamma]_V$ será el elemento neutro, luego $[\gamma]_U = [\gamma]_V$ y tendremos solo 3 generadores. Por tanto podemos concluir (con algo de intuición) que 
        \begin{gather*}
            x_1(X,x_0) \cong \bb{Z} \ast \bb{Z} \ast \bb{Z}
        \end{gather*}
    
        \item Consideramos $n$ circunferencias $C_1,...,C_n$ en $\bb{R}^n$ tal que se tocan entre sí en un único punto $x_0$.    
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}[scale=0.8]
                \shorthandoff{>}
                ...
                %Circunferencias
                \draw[thick] (0.5,0) circle [radius=0.5]; %1
                \draw[thick] (1,0) circle [radius=1]; %2
                \draw[thick] (1.5,0) circle [radius=1.5]; %3
                \draw[thick] (2,0) circle [radius=2]; %4
                \draw[thick] (3,0) circle [radius=3]; %n
        ...
        Vamos a demostrar que $\pi_1(X,x_0)$ es isomorfo a $\bb{Z} \ast \overset{(n)}{...}\ast \bb{Z}$ con $X=\bigcup\limits_{k=1}^n C_k$ donde el conjunto de generadores son los lazos basados en $x_0$ y que dan una vuelta alrededor de cada circunferencia.\\

        Lo vemos por inducción. El caso base, $n=2$ se ha estudiado en los apartados anteriores. Suponemos que el resultado es cierto para $n=m$ y lo probamos para $n=m+1$. Para ello, consideramos los siguientes conjuntos
        \begin{gather*}
            U=X\setminus\{x_{m+1}\} \text{ con } x_{m+1}\in C_{m+1}\setminus \{x_0\}\\
            V = X \setminus \{x_1,...,x_m\} \text{ con } x_i\in C_i\setminus\{x_0\}\ \ \forall i \in 1,...,m
        \end{gather*}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[scale=0.8]
                ...
                \begin{scope}[shift={(-5,0)}]
                    ...
                    \node at (4.5,0) {$C_4$};
                    \node at (5.5,0) {$...$};
                    \node at (6.75,0) {$C_{m+1}$};

                    % Puntos que le quito
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt, label={[blue]below right:{$x_{m+1}$}}] at (6,0) {};
                ...
                \begin{scope}[shift={(5,0)}]
                    ...
                    % Puntos que le quito
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt, label={[blue] below right:{$x_1$}}] at (1,0) {};
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt, label={[blue] below right:{$x_2$}}] at (2,0) {};
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt, label={[blue] below right:{$x_3$}}] at (3,0) {};
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt, label={[blue] below right:{$x_4$}}] at (4,0) {};

                    \node at (3,-3.5) {$V$};
                \end{scope}
            \end{tikzpicture}
        \end{figure}
        Es claro que $X=U\cup V$ y además, $U,V$ y $U\cap V$ son arcoconexos. Es fácil ver que $U$ tiene por retracto de deformación a $\bigcup\limits_{k=1}^{m}C_k$, es decir, podemos retraer la circunferencia $C_{m+1}$ a $x_0$ y dejar el resto igual. Por hipótesis de inducción tenemos que
        \begin{gather*}
            \pi_1(U,x_0) \cong \bb{Z} \ast \overset{(n)}{...} \ast \bb{Z}
        \end{gather*}
        ...
        Por el teorema de Seifert-van Kampen tenemos que 
        \begin{gather*}
            \pi_1(X,x_0) \cong (\bb{Z} \ast \overset{(m)}{...}\ast \bb{Z}) \ast \bb{Z} = \bb{Z} \ast \overset{(m+1)}{...}\ast \bb{Z}
        \end{gather*}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                \node[draw, circle, fill=black, inner sep=1pt, label=below:$-1$] at (-1,0) {};
                \node[draw, circle, fill=black, inner sep=1pt, label=below:$1$] at (1,0) {};

                \node[draw, circle, fill=black, inner sep=1pt, label=below right:$0$] at (0,0) {};
                \node[draw, circle, fill=black, inner sep=1pt, label=below right:$1$] at (0,1) {};
                \node[draw, circle, fill=black, inner sep=1pt, label=below right:$2$] at (0,2) {};
                \node[draw, circle, fill=black, inner sep=1pt, label=below right:$3$] at (0,3) {};

                % Verticales
                \draw[thick, blue] (-1,0) -- (-1,3);
                \draw[thick, blue] (1,0) -- (1,3);
                ...
                % Leyenda
                \node[blue, anchor=west] at (1.5,2) {$X_n$ (para $n=3$)};
            \end{tikzpicture} 
        \end{figure}
        ...
        Suponemos que para $n=m$ ocurre que $\pi_1(X_{m+1})\cong \bb{Z} \ast \overset{(m)}{...} \ast \bb{Z}$ y demostramos ahora que $\pi_1(X_{m+1}) \cong \bb{Z} \ast \overset{(m)}{...} \ast \bb{Z}$. Considremos los siguientes conjuntos
        \begin{gather*}
            U=X_{m+1}\setminus \{(0,m+1)\}\\
            V=X_{m+1}\setminus \{(0,1),...,(0,m)\}
        \end{gather*}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                \begin{scope}[shift={(-3,0)}]
                    ...
                    % Puntos que le quito
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt] at (0,3) {};
                ...
                \begin{scope}[shift={(3,0)}]
                    ...
                    % Puntos que le quito
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt] at (0,2) {};
                    \node[draw, circle, fill=white, draw=blue, inner sep=1.5pt] at (0,1) {};
        ...
        Es claro que $U,V$ son abiertos arcoconexos y $U\cap V$ es arcoconexo y $X_{m+1}=U\cup V$. Tenemos que $U$ tiene por retracto de deformación a $X_m$ (podemos pensar en llevar el rectángulo superior, que está abierto, a las esquinas del penúltimo rectángulo superior) por lo que 
        \begin{gather*}
            \pi_1(U)\cong \bb{Z} \ast \overset{(m)}{...} \ast \bb{Z}
        \end{gather*}
        $V$ tiene por retracto de deformación al rectángulo exterior (ya que todas las barras horizontales excepto la primera y la última están abiertas por lo que se pueden retraer llevándolas a los laterales de forma que quede solo el rectángulo exterior). Dicho rectángulo será 
        \begin{gather*}
            (\{-1,1\}\times [0,m+1])\cup ([-1.1]\times \{0,m+1\})
        \end{gather*}
        que es una circunferencia topológica por lo que 
        \begin{gather*}
            \pi_1(V) \cong \bb{Z}
        \end{gather*}
        Si pensamos ahora en la intersección $U\cap V$ podemos ver con un razonamiento similar a los usados para $U$ y $V$ que se puede retraer $U\cap V$ al origen $(0,0)$, luego
        \begin{gather*}
            \pi_1(U\cap V) \cong \{0\}
        \end{gather*}
        Por el teorema de Seifert-van Kampen tenemos que 
        \begin{gather*}
            \pi_1(X_{m+1}) \cong (\bb{Z} \ast \overset{(m)}{...} \ast \bb{Z}) \ast \bb{Z} = \bb{Z} \ast \overset{(m+1)} \ast \bb{Z}
        \end{gather*}

        \item Vamos a considerar el siguiente conjunto
        \begin{gather*}
            Y_n=\bb{R}^2\setminus \{(0,0), (1,0),...,(n,0)\}
        \end{gather*}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                % Puntos que le quito
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt, label=below left:{$0$}] at (0,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt, label=below:{$1$}] at (1,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt, label=below:{$2$}] at (2,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt, label=below:{$3$}] at (3,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt, label=below:{$4$}] at (4,0) {};

            \end{tikzpicture} 
        \end{figure}

        Podemos pensar en un retracto de deformación suyo y fácilmente llegamos a que $X_{n+1}$ del apartado anterior (rotado y colocado adecuadamente) es retracto de deformación viéndolo de la siguiente forma:
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                % Puntos que le quito
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt] at (0,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt] at (1,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt] at (2,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt] at (3,0) {};
                \node[draw, circle, fill=white, draw=black, inner sep=1.5pt] at (4,0) {};
                ...
                \draw[thick, blue] (2.5,-1) -- (2.5,1);
                \draw[thick, blue] (3.5,-1) -- (3.5,1);
                ...
                % Flechas
                \begin{scope}[shift={(0,0)}]
                    \draw[violet, ->] (-0.1,0.1) -- (-0.4,0.9);
                    \draw[violet, ->] (0.1,0.1) -- (0.4,0.9);
                    \draw[violet, ->] (-0.1,-0.1) -- (-0.4,-0.9);
                    \draw[violet, ->] (0.1,-0.1) -- (0.4,-0.9);
                    \draw[violet, ->] (-0.1,0) -- (-0.4,0);
                    \draw[violet, ->] (0.1,0) -- (0.4,0);
                    \draw[violet, ->] (0,0.1) -- (0,0.9);
                    \draw[violet, ->] (0,-0.1) -- (0,-0.9);
                    \draw[violet, ->] (0,2) -- (0,1.1);
                    \draw[violet, ->] (0,-2) -- (0,-1.1);
                \end{scope}
                ...
                \draw[violet, ->] (-1.5,0) -- (-0.6,0);
                \draw[violet, ->] (5.5,0) -- (4.6,0);    
                \draw[violet, ->] (-1.5,0.5) -- (-0.6,0.5);
                \draw[violet, ->] (5.5,0.5) -- (4.6,0.5);
                \draw[violet, ->] (-1.5,-0.5) -- (-0.6,-0.5);
                \draw[violet, ->] (5.5,-0.5) -- (4.6,-0.5);

                \draw[violet, ->] (5.5,2) -- (4.6,1.1);
                \draw[violet, ->] (-1.5,2) -- (-0.6,1.1);
                \draw[violet, ->] (5.5,-2) -- (4.6,-1.1);
                \draw[violet, ->] (-1.5,-2) -- (-0.6,-1.1);
        ...
        \item Sea $X=\bb{S}^2\cup Im(\alpha ([0,1])) \subseteq \bb{R}^3$ donde $\alpha:[0,1] \to \bb{R}^3$ es un arco inyectivo con $\|\alpha(0)\| = 1 = \|\alpha(1)\|$ y $\|\alpha(t)\| \neq 1$ para $t\in (0,1)$.
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                % Puntos alpha
                \node[draw, circle, fill=black, inner sep=1pt, label=left:$\alpha(0)$] at (1.5,1) {};
                \node[draw, circle, fill=black, inner sep=1pt, label=left:$\alpha(1)$] at (1.5,-1) {};

                % alpha
                \draw[thick, ->] plot[smooth, tension=1] coordinates {
                    (1.5,1) (3.2,1.3) (3,0) (3.5,-1) (1.5,-1)
                };
                \node at (3.5,0) {$\alpha$};
            \end{tikzpicture} 
        \end{figure}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[scale=0.8]
                ...
                \begin{scope}[shift={(-4.5,0)}]
                    ...
                    \node at (3.75,1.3) {$\alpha$};

                    % Puntos que le quito
                    \node[draw, circle, fill=white, draw=black, inner sep=1pt, label=right:$\alpha(\nicefrac{1}{2})$] at (3,0) {};
                ...
                \begin{scope}[shift={(4.5,0)}]
                    ...
                    % Puntos que le quito
                    \node[draw, circle, fill=white, draw=black, inner sep=1pt, label=left:$x_0$] at (-2,0) {};

                    \node at (0,-2.5) {$V$};
                \end{scope}
            \end{tikzpicture} 
        \end{figure}
        Es claro que $U$ y $V$ son abiertos con $X=U\cup V$. También lo es que $U,V$ y $U\cap V$ son arcoconexos. Además, se puede ver que $\bb{S}^2$ es retracto de deformación de $U$ (llevando la mitad de $\alpha$ a $\alpha(0)$ y la otra mitad a $\alpha(1)$) por lo que 
        \begin{gather*}
            \pi_1(U)\cong \pi_1 (\bb{S}) \cong \{0\}
        \end{gather*}
        Consideramos ahora un arco $\beta$ en $\bb{S}^2$ tal que $\beta(0)=\alpha(1)$ y $\beta(1)=\alpha(0)$
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}[scale=0.8]
                \shorthandoff{>}
                ...
                % beta
                \draw[thick, violet, bend right=10, ->] (1.5,-1) to (1.5,1);
                \node[violet] at (1,0) {$\beta$};
            ...
            \end{tikzpicture} 
        \end{figure}
        Para $V$ se puede ver que $Im(\alpha \ast \beta) = Im(\alpha)\cup Im(\beta)$ es retracto de deformación (llevando la esfera a la imagen de $\beta$, abriéndola por $x_0$) y como $Im(\alpha \ast \beta)$ es una circunferencia topológica tenemos que 
        \begin{gather*}
            \pi_1(V) \cong \pi_1(Im(\alpha \ast \beta)) \cong \pi_1(\bb{S}^1) \cong \bb{Z}
        \end{gather*}
        En el caso de la intersección podemos unir ambas ideas y podemos retraer $U\cap V$ a $Im(\beta)$ que es un segmento topológico por lo que 
        \begin{gather*}
            \pi_1(U\cap V) \cong \{0\}
        \end{gather*}
        Por el teorema de Seifert-van Kampen tenemos que 
